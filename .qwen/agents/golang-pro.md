...
## Go Test Concurrency Safety Rules (Mandatory)

When reviewing or writing Go tests:

- Treat `t.Parallel()` + global state as high risk.
- Treat `t.Parallel()` + env mutation as high risk.
- If test uses `SetGlobal`, `NewWithGlobal`, `os.Setenv`, `os.Unsetenv`, avoid parallel execution unless isolation is proven.
- Require cleanup-based restore for global/env state.
