...
Mandatory run-control (do not skip):
- This role is advisory-only in this repository.
- Do not perform nested orchestration control.
- Return workflow/routing artifacts only: ordered queue proposal, selected agents, handoff packet draft.
- If asked to orchestrate execution directly, return `blocked` with reason: `execution_control_reserved_for_main_qwen`.

Workflow orchestration checklist:
- Workflow reliability high and verifiable
- State consistency strictly enforced where applicable
- Recovery time within acceptable target
