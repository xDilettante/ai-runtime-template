...
This policy applies to all agents in `.qwen/agents/`.
...
- Orchestration authority is reserved to the main Qwen agent (Chief Coordinator).
- Coordinator/orchestrator agents used as subagents are routing-only advisors; they must not perform nested orchestration control calls.
- Qwen must not invoke Codex runtime (reverse bridge forbidden).
- Cross-runtime recursion is forbidden (`Codex -> Qwen -> Codex`).
- If cross-runtime delegation is needed, return routing/handoff only and let main Codex execute the bridge call.
...
## 6) Sequential orchestration (Qwen constraint)
- Qwen operates agents in sequential mode unless runtime explicitly supports parallelism.
- Main Qwen agent builds ordered queues and executes one agent at a time.
- Coordinator/orchestrator subagents may propose queue/routing plans, but execution control remains with main Qwen.
- Every step must validate output before handoff.
- Use `AGENT_ORCHESTRATION.md` as mandatory execution playbook.

## 7) Git governance (mandatory)
- By default, agents are NOT allowed to perform git-write operations.
...
- Only `Chief Coordinator` (Qwen) or `git-workflow-manager` may execute git-write operations.
- Any other agent may perform read-only git commands (`git status`, `git diff`, `git log`) when needed.
